...
`demo` 模式完全使用本地示例数据。`api` 模式优先访问真实接口，接口超时或失败时只回退相应的演示能力，底图与其他图层保持可用。
...
当前线路和业务对象均标记为示例/参考数据，不作为测绘或正式业务依据。公开发布前请替换为已核验的合规地图数据，并补充来源、版本、有效期和审图信息。
